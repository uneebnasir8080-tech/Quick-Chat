    ...
    <div className='min-h-screen bg-cover bg-no-repeat flex items-center justify-center'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-5 p-10 flex-1'>
        ...
        <label htmlFor="avatar" className='flex items-center gap-3 cursor-pointer'>
          <input type="file" id="avatar" hidden accept='.png, .jpg ,.jpeg' onChange={(e)=>setSelectedImage(e.target.files[0])} />
      ...
      <img src={authUser?.ProfilePic || assets.logo_icon} className={`max-w-44 ${selectedImg && 'rounded-full'} aspect-square rounded-full mx-10 max-sm:mt-10`} alt="" />
      </div>      
    </div>
  )
}
